...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        int count = 0;
        for (Sq pointer : model) {
            if (start.connectable(pointer)) {
                count++;
            }
        }
        if (count == 1) {
            for (Sq pointer : model) {
                if (start.connectable(pointer) && (start.direction()
                        == dirOf(start.x, start.y, pointer.x, pointer.y))) {
                    return pointer;
                }
            }
        }
        for (Sq pointer : model) {
            if ((start.direction() == dirOf(start.x,
                    start.y, pointer.x, pointer.y)
                && (start.sequenceNum() == pointer.sequenceNum() - 1)
                && start.connectable(pointer))) {
                return pointer;
            }
        }
        return null;
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        int count = 0;
        for (Sq pointer : model) {
            if (pointer.connectable(end)) {
                count++;
            }
        }
        if (count == 1) {
            for (Sq pointer : model) {
                if (pointer.connectable(end) && (pointer.direction()
                        == dirOf(pointer.x, pointer.y, end.x, end.y))) {
                    return pointer;
                }
            }
        }
        for (Sq pointer : model) {
            if ((pointer.direction() == dirOf(pointer.x,
                    pointer.y, end.x, end.y)
                    && (pointer.sequenceNum() == pointer.sequenceNum() - 1)
                    && pointer.connectable(end))) {
                return pointer;
            }
        }
        return null;
