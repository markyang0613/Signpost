...
class Model implements Iterable<Model.Sq> {
    ...
    /** creates a square in a model board with SOLUTION. */
    Model(int[][] solution) {
        ...
        for (int x = 0; x < width() - 1; x++) {
            for (int y = 0; y < height() - 1; y++) {
                _board[x][y]._successors
                        = allSuccessors(x, y, _board[x][y]._dir);
                _board[x][y]._predecessors
                        = _board[x][y].predecessors();
            }
        }
        for (Sq[] col : _board) {
            for (Sq sq : col) {
            ...
            for (int y = 0; y < height(); y++) {
                for (int i = 0; i < width(); i++) {
                    for (int j = 0; j < height(); j++) {
                        if (_board[x][y].connectable(_board[i][j])) {
                            _board[x][y].connect(_board[i][j]);
                        }
                    }
                }
            }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this._sequenceNum != 0) {
                ...
                if (s1._sequenceNum != this._sequenceNum + 1) {
                    Sq sone = s1;
                    int sequencenumber = this._sequenceNum;
                    while (sone != null) {
                        sone._sequenceNum = sequencenumber + 1;
                        sone = sone._successor;
                ...
                Sq dis = this;
                int oneless = s1._sequenceNum - 1;
                if (s1._sequenceNum != this._sequenceNum + 1) {
                    while (dis != null) {
                        dis._sequenceNum = oneless;
                        dis = dis._predecessor;
                        oneless = oneless - 1;
                    }
                }
            } else {
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                helperDisconnect(next);
            }
            for (Sq boob = next; boob != null;
                 boob = boob._successor) {
                boob._head = next;
            }
        }

        /** A helper function for disconnect that takes in a square NEXT. */
        void helperDisconnect(Sq next) {
            boolean flag = true;
            for (Sq pointer = this._predecessor;
                 pointer != null; pointer = pointer._predecessor) {
                if (pointer.hasFixedNum()) {
                    flag = false;
                }
            }
            if (!this.hasFixedNum() && flag) {
                if (this._predecessor != null) {
                    int ass = newGroup();
                    for (Sq guidance = this; guidance != null;
                         guidance = guidance._predecessor) {
                        guidance._sequenceNum = 0; guidance._group = ass;
                    }
                } else {
                    for (Sq frank = this; frank != null;
                         frank = frank._predecessor) {
                        frank._sequenceNum = 0;
                    }
                }
            } else {
                for (Sq hawaii = this; hawaii != null;
                     hawaii = hawaii._predecessor) {
                    hawaii._group = -1;
                }
            }
            boolean bitch = true;
            for (Sq facebook = next; facebook != null;
                 facebook = facebook._successor) {
                if (facebook.hasFixedNum()) {
                    bitch = false;
                }
            }
            if (!next.hasFixedNum() && bitch) {
                if (next._successor != null) {
                    int number = newGroup();
                    for (Sq picnic = next; picnic != null;
                         picnic = picnic._successor) {
                        picnic._sequenceNum = 0; picnic._group = number;
                    }
                } else {
                    for (Sq swad = next; swad != null; swad = swad._successor) {
                        swad._sequenceNum = 0;
                    }
                }
            } else {
                for (Sq aloha = next; aloha != null;
                     aloha = aloha._successor) {
                    aloha._group = -1;
                }
            }
        }
